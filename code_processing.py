...
def main():
  def test_tokenize_code_corpus():
    f = pickle.load(open("sql_code.txt", "rb"))
    sf = pickle.load(open("s.txt", "rb"))
    #print sf['train']
    questionf = pickle.load(open("sql_title.txt", "rb"))
    wf = codecs.open("train.txt", "w", 'utf-8')
    wfprocess = codecs.open("processPro.txt", "w", 'utf-8')
    num = 0
    for x in f:
        hasnl = True
        try:
        ...
        wfprocess.write(str(x[0]) + "\n")
        if hasnl:
            wfprocess.write(questionf[x[0]])
        else:
            wfprocess.write("None")
        wfprocess.write("\n")
        #wfprocess.write(str(x[1]) + "\n")
        wfprocess.write("  ".join(ans["1"]))
        wfprocess.write("\n")
        clean_code[x[1]] = ("	".join(ans["1"]), x)
    for x in sf['train']:
        try:
